...
import {
  LayoutDashboard, Calendar, DollarSign, BarChart2, Settings, LogOut, Menu, X,
        ...
        <div className={`px-3 py-3 border-t border-[var(--border)] ${collapsed ? "flex justify-center" : ""}`}>
          {currentUser && (
            <div className={`flex items-center gap-3 ${collapsed ? "" : "w-full"}`}>
              ...
              >
                <UserAvatar gender={currentUser.gender} name={fullName} size="sm" />
                {!collapsed && (
...
export function studentNavItems(pendingCount = 0): NavItem[] {
  return [
    ...
    { to: "/student/finance", label: "Finance", icon: <DollarSign size={16} /> },
...
export function adviserNavItems(pendingCount = 0): NavItem[] {
  return [
    ...
    { to: "/adviser/finance", label: "Finance", icon: <DollarSign size={16} /> },
